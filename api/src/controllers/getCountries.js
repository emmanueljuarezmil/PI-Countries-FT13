...
const getCountries = async (req, res, next) => {
  ...
  activityFilter =
    activityFilter !== "undefined" && activityFilter !== ""
      ? activityFilter
      : "";
  continentFilter =
    continentFilter !== "undefined" && continentFilter !== ""
      ? continentFilter
      : "";

  if (
    (orderBy !== "name" && orderBy !== "poblation" && orderBy !== "area") ||
  ...
  try {
    ...
    const totalPages = Math.ceil(
      Object.values(totalItems).length / itemsPerPage
    );
    ...
    const continents = [
      ...new Set(countriesContinents.map((country) => country.continent)),
    ];
