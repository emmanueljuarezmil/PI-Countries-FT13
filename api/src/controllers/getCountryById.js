...
const getCountryById = async (req, res, next) => {
  ...
  try {
    ...
    if (country) {
      ...
      while (nearbyCountries.length < 6) {
        nearbyCountries = await Country.findAll({
          where: {
            lat: {
              [Op.between]: [
                parseInt(country.lat) - latAndLngDifference,
                parseInt(country.lat) + latAndLngDifference,
              ],
            },
            lng: {
              [Op.between]: [
                parseInt(country.lng) - latAndLngDifference,
                parseInt(country.lng) + latAndLngDifference,
              ],
            },
          },
      ...
      nearbyCountries = nearbyCountries.filter(
        (nearbyCountry) => nearbyCountry.name !== country.name
      );
      return res.send({
        country,
        nearbyCountries,
